...
const useFilteredTasks = () => {
  ...
  if (search) {
    ...
    filtered = filtered.filter(
      (task) =>
        ...
        (task.tags && task.tags.some((tag) => tag.toLowerCase().includes(lowercasedSearch))),
    );
  }
  ...
  if (priority !== 'all') {
    ...
    filtered = filtered.filter(
      (task) => task.priority && task.priority.toLowerCase() === lowercasedPriority,
    );
  }
  ...
  if (sort === 'date-asc') {
    ...
    filtered.sort((a, b) => {
      const aValue = priorityValues[(a.priority || 'medium').toLowerCase()] || 0;
      const bValue = priorityValues[(b.priority || 'medium').toLowerCase()] || 0;
